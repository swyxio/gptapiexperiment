...
class ChatAssistant {
  ...
  listFunctions() {
    return Object.keys(this.registeredFunctions).map(fnName => this.getFunction(fnName));
  }
  ...
  async extraChat(messages, function_call) {
    ...
    do {
      ...
      if (selectedFunction) {
        ...
        this.logger.log('function not found', chatCompletion.data.choices[0].message.function_call);
