// this is the prototype of the functionality that is now in smol logger log to tsv. you shouldnt need this anymore except for reference
...
fs.readdir(logsDir, (err, dirs) => {
  ...
  Object.keys(headerObj["payload"]).forEach((header) =>
        ...
        Object.keys(fileObj).forEach((key) => {
          if (key === "payload") {
            ...
            if (Array.isArray(payload)) {
              ...
              Object.keys(fileObj[key]).forEach((payloadKey) =>
                row.push(JSON.stringify(fileObj[key][payloadKey]))
              );
            } else {
              // just a raw value
